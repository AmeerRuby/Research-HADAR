...
class HADARMultipleScenes():
    ...
    def __init__(self, root='/Users/nguyenvietthai/Downloads/HADAR-main/TeXNet',
                 split='train', inp_transform=None, target_transform=None,
                 **kwargs):

        root = os.path.expanduser('/Users/nguyenvietthai/Downloads/HADAR-main/TeXNet')
    ...
    def _load_data(self):
        ...
        for i in self.S_beta_files:
            ...
            print(f"Shape of S_beta_files data: {data.shape}")
        ...
        for i in self.S_files:
            ...
            # Hard-code cropping data
            data = data[:,:512,:512]
            print(f"size of data: {sys.getsizeof(data)}")
            print(f"Shape of S_files data: {data.shape}")
            # data = data[4:53]
            # data = data[4:53] # 49 channels
            # print("Shape of S = ", np.shape(data))
            # Padding
            data = np.pad(data, ((0, 0), (6, 6), (0, 0)), 'constant', constant_values=0)
            data = torch.from_numpy(data).type(torch.float)
            self.S.append(data)
        ...
        for i in self.T_files:
            ...
            # Padding
            data = np.pad(data, ((6, 6), (0, 0)), 'constant', constant_values=0)
            data = torch.from_numpy(data).type(torch.float)
            print(f"Shape of T data: {data.shape}")
        ...
        for i in self.e_files:
            ...
            data = np.pad(data, ((6, 6), (0, 0)), 'constant', constant_values=0)
            data = torch.from_numpy(data).type(torch.long)
            print(f"Shape of e data: {data.shape}")
            self.eMaps.append(data)
        ...
        for i in self.v_files:
            ...
            data = np.pad(data, ((0, 0), (6, 6), (0, 0)), 'constant', constant_values=0)
            data = torch.from_numpy(data).type(torch.float)
            print(f"Shape of v data: {data.shape}")
    ...
    def __getitem__(self, index_):
        ...
        if self.split == 'train' or (self.split == "val" and size_idx == 0):
            ...
            crop_size = 512#256
            w, h = S.shape[1:]
            th, tw = 512,512#256, 256 ## Hardcoding crop sizes
            
            # Hardcoding fix crop
            x1 = 0#random.randint(0, w - tw)
            y1 = 0#random.randint(0, h - th)
